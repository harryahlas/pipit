...
def show_phases(pipit):
    ...
    brain = pipit.brain
    R = np.sqrt(brain.mem_cos**2 + brain.mem_sin**2)  # (n_osc, VOCAB)
    total_R = R.sum(axis=1)  # total entrainment per oscillator
    order = np.argsort(total_R)[::-1]
    parts = []
    for i in order[:6]:
        period = abs(2 * np.pi / brain.omega[i]) if abs(brain.omega[i]) > 1e-6 else float('inf')
        parts.append(f"osc[{i}] T≈{period:.1f} R=[{R[i,0]:.2f},{R[i,1]:.2f}]")
    print(f"    entrainment: {', '.join(parts)}")


def train(name='p1', seed=0, rounds=5000, out='babies',
          n_osc=16, per_class=200, length=64, eval_per_class=20,
    ...
    print(f"\n=== birthing pipit '{name}' ===")
    pipit = Pipit(name=name, n_osc=n_osc, seed=seed)
